...
local INTRO_LOGO_WIDTH  = 80

local autocmd_group = vim.api.nvim_create_augroup('singul4ri7y', {})
local highlight_ns_id = vim.api.nvim_create_namespace('singul4ri7y')
local intro_buff = -1
...
local function create_and_set_intro_buf(default_buff)
	...
	vim.api.nvim_buf_set_name(intro_buff, 'singul4ri7y')
...
local function display_intro(payload)
	...
	if not is_dir and default_buff_name ~= '' and default_buff_filetype ~= 'singul4ri7y' then
		return
	end
